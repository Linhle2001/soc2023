...
class shipment(models.Model):
 ### The following are the fields of our table.
    ...
    def __str__(self):
        return '%s %s %s %s %s %s %s %s %s %s' %(self.fname, self.lname, self.email, \
            self.mobile, self.product_id, self.address, self.quantity , self.payment_status, \
                self.transaction_id, self.shipment_status)